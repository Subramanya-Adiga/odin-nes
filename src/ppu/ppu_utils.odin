...
get_color_from_palette :: proc(ppu: ^PPU, palette: u8, color: u8) -> sdl2.Color {
	return(
		ppu.pal_screen[ppu_read(&ppu.bus, (u16(0x3F00) + u16(palette << 2) + u16(color))) & 0x3F] \
	)
...
get_pattern_table :: proc(ppu: ^PPU, index: u8, palette: u8) -> ^sdl2.Surface {
	for tile_y in 0 ..< 16 {
		for tile_x in 0 ..< 16 {
			...
			for row in 0 ..< 8 {
				loc := u16(index) * 0x1000 + u16(offset + row)
				...
				for col in 0 ..< 8 {
					...
					tile_lsb >>= 1
					tile_msb >>= 1
					rect := sdl2.Rect{1, 1, i32((tile_x * 8) + (7 - col)), i32((tile_y * 8) + row)}
					sdl2.FillRect(
